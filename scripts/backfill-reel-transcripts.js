...
// transcribes the Czech audio. Returns the plain transcript text, or '' on
// any failure / silent clip. Temp files are always cleaned up.
async function transcribeVideo(mp4Url) {
  ...
  try {
    ...
    const body = {
      contents: [{ parts: [
        { text: 'Přepiš přesně mluvené slovo z tohoto audia do češtiny. Vrať pouze čistý přepis bez komentáře. Pokud audio neobsahuje žádnou řeč, vrať prázdný řetězec.' },
...
async function main() {
  ...
  // Every FB video/reel row that does not yet carry a transcript. Both permalink
  // shapes (/reel/ and /videos/) are matched. Paged in case of >1000 rows.
  const rows = [];
  const PAGE = 1000;
  ...
  console.log(`Found ${rows.length} video rows without transcript; processing ${targets.length}.`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if (!mp4Url) { noAudio++; continue; }

    const transcript = await transcribeVideo(mp4Url);
    await sleep(150);
    if (!transcript) { noAudio++; continue; }
    ...
    updated++;
    process.stdout.write(`\r  ${i + 1}/${targets.length}: ${updated} transcribed, ${noAudio} no audio...`);
  }

  process.stdout.write('\n');
  console.log(`Done. ${updated} videos transcribed. ${noAudio} had no usable audio.`);
}
