  ...
  try {
    const [vectorRes, insRes, suppRes, rentRes] = await Promise.all([
      session.run(`
        ...
        YIELD node MATCH (node)-[:PART_OF]->(d:Document)
        RETURN collect(DISTINCT { text: node.text, src: d.human_name, url: d.url }) AS chunks
      `, { vec: vector }),
      session.run(`
        CALL db.index.fulltext.queryNodes("insuranceSearch", $term) 
        YIELD node, score RETURN node.search_text as text ORDER BY score DESC LIMIT 3
      `, { term: question }),
      session.run(`
        CALL db.index.fulltext.queryNodes("supplierSearch", $term) 
        YIELD node RETURN node.name + ' (' + node.city + ')' AS text LIMIT 3
      `, { term: question }),
      session.run(`
        CALL db.index.fulltext.queryNodes("rentalSearch", $term) 
        YIELD node RETURN node.name + ' (' + node.city + ')' AS text LIMIT 3
      `, { term: question })
    ]);
    return {
