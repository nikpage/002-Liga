...
async function getFullContext(vector, question) {
  ...
  try {
    ...
    const [vectorRes, insRes, suppRes, rentRes] = await Promise.all([
      ...
      session.run(`CALL db.index.fulltext.queryNodes("supplierSearch", $term) YIELD node RETURN node.name + ' (' + node.city + ')' AS text LIMIT 3`, { term: question }),
      session.run(`CALL db.index.fulltext.queryNodes("rentalSearch", $term) YIELD node RETURN node.name + ' (' + node.city + ')' AS text LIMIT 3`, { term: question })
    ]);
    return {
